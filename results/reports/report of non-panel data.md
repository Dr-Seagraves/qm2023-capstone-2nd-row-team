## Information Regarding the Attempts at Regressing a Time Series

**Note: The following document was created during a failed attempt to run FE and DiD analysis on non-panel data (sentiment and market returns). Look at the actual regressions for files with the M3v2 label. We believe some meaning can be derived from this report, therefore it is not being deleted.**
